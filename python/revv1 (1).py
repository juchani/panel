...
def monitoreo():
    ####################PANEL 4 (FIJO)
    a=ModbusClient(host="192.168.0.66", port=502, unit_id=1, auto_open=True)################PANEL FIJO#####################  
    regs = a.read_holding_registers(109, 4)
    if(regs is not None):
        if(regs[1]is not 0 and regs[0]is not 0):
            data = []
    ...
    ####################PANEL 2 (EJE Y)
    b=ModbusClient(host="192.168.0.73", port=502, unit_id=1, auto_open=True)################PANEL FIJO#####################  
    ...
    if(regs1 is not None):
        if(regs1[1]is not 0 and regs1[0]is not 0):
            ...
            cliente.write_points(data, database='', time_precision='s', protocol='line')
     ####################PANEL 3 (EJE X)
    c=ModbusClient(host="192.168.0.23", port=502, unit_id=1, auto_open=True)################PANEL FIJO#####################  
    ...
    if(regs2 is not None):
     ...
     ####################PANEL 1 (EJE X-Y)
    d=ModbusClient(host="192.168.0.3", port=502, unit_id=1, auto_open=True)################PANEL FIJO#####################  
    regs3 = d.read_holding_registers(109, 4)
    if(regs3 is not None):
        if(regs3[1]is not 0 and regs3[0]is not 0):
            print(regs3)
            data = []
